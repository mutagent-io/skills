...
[`references/eval-criteria.md`](./references/eval-criteria.md) §
...
3. **NEVER auto-generate eval criteria -- collect from user.** Ask the user for each rubric field. See [references/eval-criteria.md](./references/eval-criteria.md) for the rubric format.
...
|---|---|---|---|
| Single LLM call -> text/JSON output | Prompt Optimization | `mutagent prompts *` | [references/optimization.md](./references/optimization.md) |
| Multi-turn / tool-calling / state graph | Agent (WIP) | `mutagent agents *` (CRUD only) | [references/agents.md](./references/agents.md) (stub) |
...
> `references/eval-criteria.md` for the rubric design framework AND
> `references/eval-creation.md` for the step-by-step CLI sequence). Each topic's
...
|---|---|---|
| "trace", "observe", "integrate", "add framework" | [references/tracing.md](./references/tracing.md) | Non-destructive, fastest first-value path |
| "optimize", "improve", "tune", "upload prompt" | [references/optimization.md](./references/optimization.md) | Full create->dataset->eval->optimize loop (orchestrator) |
| "create dataset", "add examples", "test cases", "edge cases", "hard cases", "expand dataset", "dataset items" | [references/dataset-curation.md](./references/dataset-curation.md) (HOW) + [references/dataset-design.md](./references/dataset-design.md) (WHY) | Standalone dataset curation (no optimization context needed) |
| "create evaluation", "create rubric", "evaluate prompt", "judge", "score this prompt", "rubric design", "MVC", "Output Standards" | [references/eval-creation.md](./references/eval-creation.md) (HOW) + [references/eval-criteria.md](./references/eval-criteria.md) (WHY) | Standalone evaluation rubric creation (no optimization context needed) |
| "explore", "scan", "find prompts", "what prompts", "discover" | [references/exploration.md](./references/exploration.md) | Read-only discovery + taxonomy |
| `AgentExecutor`, `StateGraph`, `createReactAgent`, `tool_calls`, `@tool`, `langgraph`, `crewai`, `autogen`, `openai/agents`, multi-turn | [references/agents.md](./references/agents.md) | WIP path -- surface partnership link |
| "how do variables work", "single vs double braces", delimiter | [references/prompt-variables.md](./references/prompt-variables.md) | Delimiter inference contract (concept-only; prompt creation lives inline in optimization.md step 4) |
| "what makes a good eval" (concept question only, no creation intent) | [references/eval-criteria.md](./references/eval-criteria.md) | INPUT MVC + OUTPUT Standards (no workflow load) |
| "what makes a good dataset" (concept question only, no creation intent) | [references/dataset-design.md](./references/dataset-design.md) | Dataset curation principles + case categories (no workflow load) |
| "scorecard", "interpret results", "what does X score mean" | [references/scorecard-output.md](./references/scorecard-output.md) | Interpretation only (no workflow needed) |
...
|---|---|---|---|
| [references/tracing.md](./references/tracing.md) | User wants to add framework tracing / observability | Non-destructive append-only integration sequence | Must run explore first (Rule 4) |
| [references/optimization.md](./references/optimization.md) | User wants to optimize or evaluate a prompt | Full loop: explore -> upload -> dataset -> eval -> optimize -> apply | Must check usage before optimize (Rule 5); must collect rubrics from user (Rule 3) |
| [references/dataset-curation.md](./references/dataset-curation.md) | User wants to create/expand a dataset (standalone) | Focused dataset curation without full optimization context | Hard cases first; ask per-field questions |
| [references/eval-creation.md](./references/eval-creation.md) | User wants to create/edit evaluation rubric (standalone) | Focused per-field rubric collection without full optimization context | INPUT MVC + OUTPUT Standards split; ask per-field questions; collect from user (Rule 3) |
| [references/exploration.md](./references/exploration.md) | User wants to scan codebase, identify prompts vs agents | Read-only discovery; output taxonomy to user | Run only; no writes |
| [references/agents.md](./references/agents.md) | Multi-turn / tool-calling code detected | WIP -- do NOT attempt optimizer, surface partnership link | Show WIP card to user verbatim |
| [references/prompt-variables.md](./references/prompt-variables.md) | Any question about `{var}` vs `{{var}}`, delimiter inference | Brace convention + conversion rules | Load before `prompts create` in optimization workflow |
| [references/eval-criteria.md](./references/eval-criteria.md) | Any question about rubric design, MVC, Output Standards | granular rubric format -- INPUT-param vs OUTPUT-param scope | Load before `evaluation create --guided` in optimization workflow |
| [references/dataset-design.md](./references/dataset-design.md) | Any question about dataset quality, case categories, hard cases | Dataset design principles -- parallel structure to eval-criteria.md | Load before `dataset add --guided` |
