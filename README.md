...
<p align="center">
  <strong>Official MutagenT skills for AI coding agents.</strong>
</p>
...
spec](https://agentskills.io/specification)). The current skill teaches an
agent how to set up the MutagenT CLI, install lifecycle packages, inspect
workspace and usage information, and send product feedback.
...
The marketplace ships one bundled plugin (`mutagent`) that contains every skill. Releases are tagged at the plugin level: [`mutagent/v0.1.208`](https://github.com/mutagent-io/skills/releases/tag/mutagent/v0.1.208).

| Skill | Status | Path | Description |
|---|---|---|---|
| [`mutagent-cli`](./skills/mutagent-cli/) | ✅ Stable | `skills/mutagent-cli/` | CLI setup, package installation, workspace and usage discovery, and product feedback via [`@mutagent/cli`](https://www.npmjs.com/package/@mutagent/cli). |
...
auto-invoked when the agent detects matching trigger phrases (e.g. *"set up
MutagenT"*, *"install diagnostics"*). Skill namespace: `/mutagent:mutagent-cli`.
...
│   └── workflows/                # focused CLI task workflows
│       ├── setup.md
│       ├── install.md
│       └── feedback.md
├── skills/
...
- **Read `--help` before first use** — the live CLI owns its flags.
- **Login before gated commands** — authenticate before install and discovery.
- **Show command results** — do not hide meaningful CLI output in tool logs.
- **Confirm before writes** — installs, skills, and hooks require consent.
...
The final automated-mirror refresh, `mutagent/v0.1.208`, was synced from
`@mutagent/cli@0.1.208`. The daily synchronization workflow has been retired;
this repository is now a public snapshot. Any future update must be explicitly
requested and reviewed as a one-time PR.
...
internal-monorepo references that slip in during a manual import.
