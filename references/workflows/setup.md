...
## 2. init (project wizard)

```bash
mutagent init --help
mutagent init --json     # interactive project setup
```

`init` walks the user through wiring MutagenT into the current project. Read
`--help` for the current flags and present the wizard's questions to the user.
